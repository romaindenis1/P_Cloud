...
export default {
  ...
  async mounted() {
    try {
      ...
      const response = await AddBookService.getBookById(bookId)
  ...
  methods: {
    async updateBook() {
      try {
        ...
        const bookId = this.$route.params.id
        const response = await AddBookService.updateBook(bookId, formData)
        console.log('Updated Book:', response.data)
        this.$router.push('/bibliotheque')
      } catch (error) {
        console.error('Error while updating book:', error.response?.data || error.message)
      }
    },
...
<template>
  ...
  <div v-if="isLoading" style="text-align: center; padding: 2rem">Chargement...</div>
  <div v-else class="add-book">
    <div class="book-form">
      ...
      <form @submit.prevent="updateBook">
        ...
        <button id="send-book" type="submit">Mettre à jour</button>
      </form>
    </div>
  </div>
</template>
